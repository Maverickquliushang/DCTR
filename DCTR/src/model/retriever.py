...
class Retriever(nn.Module):
    ...
    def dsrs_loss(predicted_scores: torch.Tensor,
                  pseudo_label_scores: torch.Tensor,
                  beta: float = 1.0) -> torch.Tensor:

        if predicted_scores.dim() == 1:
            predicted_scores = predicted_scores.unsqueeze(0)
        if pseudo_label_scores.dim() == 1:
            pseudo_label_scores = pseudo_label_scores.unsqueeze(0)

        assert predicted_scores.shape == pseudo_label_scores.shape, \
            "Predicted scores and pseudo-label scores must have the same shape."

        batch_size, n_items = predicted_scores.shape

        indices = torch.arange(n_items, device=predicted_scores.device)
        pair_indices = torch.combinations(indices, 2)
        ...
        s_i = torch.index_select(predicted_scores, 1, i_indices)
        s_j = torch.index_select(predicted_scores, 1, j_indices)
        q_i = torch.index_select(pseudo_label_scores, 1, i_indices)
        q_j = torch.index_select(pseudo_label_scores, 1, j_indices)

        delta_s = s_i - s_j
        delta_q = q_i - q_j

        agreement_term = beta * delta_s * delta_q


        concordance_prob = torch.sigmoid(agreement_term)

        pairwise_loss = 1 - concordance_prob
